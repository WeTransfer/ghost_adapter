...
Gem::Specification.new do |spec|
  ...
  spec.metadata['allowed_push_host'] = 'https://rubygems.pkg.github.com/WeTransfer'
  spec.metadata['homepage_uri'] = spec.homepage
  spec.metadata['source_code_uri'] = spec.homepage
  spec.metadata['github_repo'] = 'ssh://github.com/WeTransfer/ghost_adapter'
  spec.metadata['rubygems_mfa_required'] = 'true'

  # Specify which files should be added to the gem when it is released.
  # The `git ls-files -z` loads the files in the RubyGem that have been added into git.
  spec.files = Dir.chdir(File.expand_path(__dir__)) do
    `git ls-files -z`.split("\x0").select { |f| File.file?(f) && !f.match?(%r{^(test|spec|features|doc|bin)/}) }
  end
  spec.require_paths = ['lib']

  spec.add_dependency 'activerecord', '>= 7.2', '< 8.0'
